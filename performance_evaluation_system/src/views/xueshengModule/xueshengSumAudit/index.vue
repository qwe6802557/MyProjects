<template>
  <div class="app-container">
    <el-table v-loading="listLoading" :data="list" highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="学科建设总分">
        <template slot-scope="scope">
          <span>{{ scope.row.jobID}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="研究生工作总分">
        <template slot-scope="scope">
          {{ scope.row.station}}
        </template>
      </el-table-column>
      <el-table-column  align="center" label="人才引进总分">
        <template slot-scope="scope">
          <span>{{ scope.row.teachingMoudle.workLoad ? scope.row.teachingMoudle.workLoad.teachWorkSum : 0}}</span>
        </template>
      </el-table-column>
      <el-table-column  align="center" label="本模块总分">
        <template slot-scope="scope">
          <span>{{ scope.row.teachingMoudle.workLoad ? scope.row.teachingMoudle.workLoad.teachWorkSum : 0}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="权重记分">
        <template slot-scope="scope">
