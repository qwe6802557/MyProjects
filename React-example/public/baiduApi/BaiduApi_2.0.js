(function () {
    ...
    // merge configuration
    Object.assign(config, window.__BMAP_EXTRA_CONFIG__);
    window.__BMAP_EXTRA_CONFIG__ = config;
})();
...
window.BMAP_AUTHENTIC_KEY = "eGgfhshrvZANxKGXHKfaGK3YBWcXOgYN";
(function() {
    function aa(a) {
        throw a;
    }
    ...
    function Ra(a, b) {
        /**
         * Don't request if enable is true
         */
        if (a && !window.__BMAP_EXTRA_CONFIG__.enable) {
            var b = b || {},
                c = "",
                d;
    ...
    /**
     * replace Baidu CDN with specified local host and path for static resources
     */
    (function (C) {
        if (C.enable) {
            z.ma = C.host + C.path;
    ...
    function(a, b, c) {
        ...
        function(a, b, c) {
            ...
            function n(a, b) {
                /**
                 * Dont't request if enable is true
                 */
    ...
    function pa(a, b) {
        /**
         * BMap requests interceptor
         * do nothing if return true
         */
        if (window.__BMAP_EXTRA_CONFIG__.enable && window.__BMAP_EXTRA_CONFIG__.interceptor(a)) return;
        if (b) {
    ...
    x.object.extend(I, {
        ...
        oL: function() {
            var a = "canvablepath";
            var b = z.GU ? "newvectordrawlib" : "vectordrawlib";
            /**
             * always use `newvectordrawlib`, not randomly
             */
            if (window.__BMAP_EXTRA_CONFIG__.enable) {
                b = "newvectordrawlib";
            }
        ...
        gG: {
            /**
             * remove querystring if enable is true
             */
            XP: z.ma + "getmodules" + (window.__BMAP_EXTRA_CONFIG__.enable ? '' : '?v=2.0&t=20140707'),
            xU: 5E3
        },
        BC: q,
        Pd: {
                ...
                if (d.Bd == this.Fj.nG) {
                    ...
                    e.BC == q && (e.BC = o, setTimeout(function() {
                        for (var a = [], b = 0, c = e.Pd.Cn.length; b < c; b++) {
                            var d = e.Pd.Cn[b],
                                l = "";
                            ja.Iy.dK(d) ? l = ja.Iy.get(d) 
                                : (
                                    l = "", 
                                    /**
                                     * remove modules name's hash if enable is true
                                     */
                                    a.push(d + (window.__BMAP_EXTRA_CONFIG__.enable ? '' : "_" + Sb[d]))
                                );
                            e.Pd.Qv.push({
                                QM: d,
                                IE: l
                        ...
                        /**
                         * load modules
                         * due to BMap modules sequences aren't certain
                         * mapping BMap wants to request modules to local server existed modules
                         */
                        (function (C) {
                            if (C.enable && a.length > 0) {
                                var reqMods = a;
                                var configMods = C.modules;
                                var configModMap = {};
                                var willReqMods = [];
                                configMods.forEach(function (modFile) {
                                    var mods = modFile.split(',');
                                    mods.forEach(function (mod) {
                                        configModMap[mod] = modFile;
                                    });
                                });
    ...
    x.extend(Gc.prototype, {
        ...
        mI: function(a) {
            ...
            for (var g = 0; g < c; g++) {
                ...
                if (!i.$H && !(i.mp && !this.map.Vb() || i.jM && this.map.Vb())) {
                    ...
                    w = [];
                    for (d.JB = []; n < v; n++)
                        for (y = l; y < e; y++) w.push([n, y]), d.JB.push({
                            x: n,
                            y: y
                        });
        ...
        Qt: function(a) {
            var b = this.map.pa();
            if (!this.map.Vb() && (a ? this.map.K.s_ = a : a = this.map.K.s_, a))
    ...
    Zc.getTilesUrl = function(a, b, c) {
        ...
        /**
         * replace tile url with local server tile pic's file name
         */
        return (function (C) {
            if (C.enable) {
                var url = C.host + C.path + C.tilePath 
                    + "/qt=tile&x=" + (d + "").replace(/-/gi, "M") + "&y=" + (a + "").replace(/-/gi, "M")
                    + "&z=" + b + "&styles=" + c + "&scaler=" + f 
                    + "&udt=" + C.tileUdt
                    + ".png";
    ...
    ad.getTilesUrl = function(a, b) {
        var c = a.x,
            d = a.y;
        /**
         * replace satellite url with local server tile pic's file name
         */
        return (function (C) {
            if (C.enable) {
                var url = C.host + C.path + C.satellitePath
                    + "/u=x=" + c + ",y=" + d + ",z=" + b
                    + ",v=009,type=sate&fm=46&udt=" + C.satelliteUdt
                    + ".jpg";
                return url.replace(/-(\d+)/gi, "M$1");
            } else {
    ...
    bd.getTilesUrl = function(a, b) {
        ...
        /**
         * replace satellite street url with local server tile pic's file name
         */
        return (function (C) {
            if (C.enable) {
                var url = C.host + C.path + C.roadPath 
                    + "/qt=tile&x=" + (c + "").replace(/-/gi, "M") + "&y=" + (d + "").replace(/-/gi, "M")
                    + "&z=" + b + "&styles=sl"
                    + "&udt=" + C.roadUdt
                    + ".png";
                return url.replace(/-(\d+)/gi, "M$1");
