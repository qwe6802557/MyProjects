      ...
      <div class="login-form-content">
        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
          ...
          <el-form-item class="button-item">
            ...
            <el-button type="danger" round class="login-button" @click="toReset" >重置</el-button>
          </el-form-item>
        </el-form>
      </div>
    </div>
...
import { reqLogin } from '@/api/admin/login'
Vue.use(Form).use(Input).use(FormItem)
const GVerify = memoryUntil.getCode(window, document)
export default {
  name: 'Login',
  data () {
    return {
  ...
  methods: {
    toLogin () {
      ...
      this.$refs['ruleForm'].validate((boolean) => {
        if (boolean === true) {
          const { adminName, password, validateWord } = this.ruleForm
          ...
          reqLogin({
            ...
            code === 1 && this.$message.success('登录成功!')
            code === 1 && this.$router.push({ name: 'Layout' })
            code === 302 && this.$message.error('用户名账号或密码错误!')
            code === 302 && this.getValiateImg()
            this.loading = false
          })
        } else {
          this.loading = false
